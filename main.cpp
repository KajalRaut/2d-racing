...
void display()
{
   ...
   std::fstream myfile("F:\\ALL\\New folder\\CodeBlocks_program\\data.txt", std::ios_base::in);
   myfile >> highscore ;
   score = score + 1;
    glColor3f(1,1,1);
    drawText("High score:", 0,455);
    ...
    drawTextNum(buffer, 6, 80,455);
    glutSwapBuffers();
    for(q = 0; q<= 10000000; q++){;}
    if(collide == true)
    {
        ofstream myfile;
  myfile.open ("F:\\ALL\\New folder\\CodeBlocks_program\\data.txt");
  myfile << highscore;
  myfile.close();
        glColor3f(0,0,0);
